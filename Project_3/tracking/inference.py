        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.legalPositions:
            ...
            for newPos, probability in newPosDist.items():
                allPossible[newPos] = allPossible[newPos] + (self.beliefs[p] * probability)
        allPossible.normalize()
        self.beliefs = allPossible
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        self.particles = []
        numOfPos = self.numParticles / len(self.legalPositions)
    ...
    def elapseTime(self, gameState):
        ...
        for p in self.particles:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, p))
            for newPos, probability in newPosDist.items():
                allPossible[newPos] = allPossible[newPos] + probability
        allPossible.normalize()
        self.beliefs = allPossible
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        while i < self.numParticles:
            if j >= len(product):
                j = 0
            self.particles.append(product[j])
            j = j + 1
            i = i + 1
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        for part in self.particles:
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i] != None:
                    trueDistance = util.manhattanDistance(part[i], pacmanPosition)
                    probability = probability * (emissionModels[i][trueDistance])
            allPossible[part] = allPossible[part] + probability
        self.beliefs = allPossible
        if self.beliefs.totalCount() == 0:
            self.initializeParticles()
        else:

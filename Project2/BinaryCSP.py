        ...
        if not assignment.isAssigned(var):
            ...
            for constraint in csp.binaryConstraints:
                if constraint.affects(var) and not assignment.isAssigned(constraint.otherVariable(var)):
                    degree = degree + 1
            remainingVals = len(domains[var])
            if remainingVals < mrv:
                mrv = remainingVals
                nextVar = var
            if remainingVals == mrv and degree > minDegree:
                minDegree = degree
                nextVar = var
    return nextVar
...
def leastConstrainingValuesHeuristic(assignment, csp, var):
    ...
    for value in values:
        ...
        for constraint in csp.binaryConstraints:
            if constraint.affects(var):
                for otherVal in assignment.varDomains[constraint.otherVariable(var)]:
                    if not constraint.isSatisfied(otherVal, value):
                        priority = priority + 1
	...
	for constraint in csp.binaryConstraints:
		if constraint.affects(var):
			...
			if not assignment.isAssigned(other):
				temp = domains[other]
				for otherVal in temp:
					if not constraint.isSatisfied(otherVal, value):
						if len(domains[other]) <= 1:
							return None
						else:
							inferences.add((other, otherVal))
...
def recursiveBacktrackingWithInferences(assignment, csp, orderValuesMethod, selectVariableMethod, inferenceMethod):
	...
	for val in orderValuesMethod(assignment, csp, var):
		if consistent(assignment, csp, var, val):
			...
			result = recursiveBacktrackingWithInferences(assignment, csp, orderValuesMethod, selectVariableMethod,
														 inferenceMethod)
...
def maintainArcConsistency(assignment, csp, var, value):
	...
	while len(arcQueue) != 0:
		head, tail = arcQueue.pop()
		currentConstraint = None
		for constraint in csp.binaryConstraints:
			if constraint.affects(head) and constraint.affects(tail):
				currentConstraint = constraint
		revisedInferences = revise(assignment, csp, head, tail, currentConstraint)
		if revisedInferences is None:
			for reverseVar, reverseVal in list(inferences):
				domains[reverseVar].update(reverseVal)
			return None
		if len(revisedInferences) != 0:
			for constraint in csp.binaryConstraints:
				if constraint.affects(tail):
					arcQueue.append((tail, constraint.otherVariable(tail)))
...
def AC3(assignment, csp):
	...
	while len(arcQueue) != 0:
		head, tail, currentConstraint = arcQueue.pop()
		revisedInferences = revise(assignment, csp, head, tail, currentConstraint)
		if revisedInferences is None:
			return None
		if len(revisedInferences) != 0:
			for constraint in csp.binaryConstraints:
				if constraint.affects(tail):
					arcQueue.append((tail, constraint.otherVariable(tail), constraint))
	return assignment
